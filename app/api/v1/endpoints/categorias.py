...
def read_categorias(
    ...
    return crud.categorias.get_multi(db, skip=skip, limit=limit)


@router.post("/", response_model=CategoriaOut, status_code=status.HTTP_201_CREATED)
def create_categoria(*, categoria_in: CategoriaCreate, db: Session = Depends(get_db)) -> CategoriaOut:
    existente = crud.categorias.get_by_nombre(db, categoria_in.nombre)
    if existente:
        detail = error_detail(
            "categoria_duplicate",
    ...
    try:
        return crud.categorias.create(db, obj_in=categoria_in)
...
def read_categoria(*, categoria_id: int, db: Session = Depends(get_db)) -> CategoriaOut:
    categoria = crud.categorias.get(db, categoria_id)
    if not categoria:
        detail = error_detail(
            "categoria_not_found",
...
def update_categoria(*, categoria_id: int, categoria_in: CategoriaUpdate, db: Session = Depends(get_db)) -> CategoriaOut:
    categoria = crud.categorias.get(db, categoria_id)
    if not categoria:
        detail = error_detail(
            "categoria_not_found",
...
def delete_categoria(*, categoria_id: int, db: Session = Depends(get_db)) -> Response:
    categoria = crud.categorias.get(db, categoria_id)
    if not categoria:
        detail = error_detail(
            "categoria_not_found",
