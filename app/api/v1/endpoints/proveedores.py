...
def read_proveedores(
    ...
    return crud.proveedores.get_multi(db, skip=skip, limit=limit)


@router.post("/", response_model=ProveedorOut, status_code=status.HTTP_201_CREATED)
def create_proveedor(*, proveedor_in: ProveedorCreate, db: Session = Depends(get_db)) -> ProveedorOut:
    existente = crud.proveedores.get_by_nombre(db, proveedor_in.nombre)
    if existente:
        detail = error_detail(
            "proveedor_duplicate",
    ...
    try:
        return crud.proveedores.create(db, obj_in=proveedor_in)
...
def read_proveedor(*, proveedor_id: int, db: Session = Depends(get_db)) -> ProveedorOut:
...
def update_proveedor(*, proveedor_id: int, proveedor_in: ProveedorUpdate, db: Session = Depends(get_db)) -> ProveedorOut:
    proveedor = crud.proveedores.get(db, proveedor_id)
    if not proveedor:
        detail = error_detail(
            "proveedor_not_found",
...
def delete_proveedor(*, proveedor_id: int, db: Session = Depends(get_db)) -> Response:
    proveedor = crud.proveedores.get(db, proveedor_id)
    if not proveedor:
        detail = error_detail(
            "proveedor_not_found",
